...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/grafana/grafana/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
		...
		$url = "https://s3-us-west-2.amazonaws.com/grafana-releases/release/grafana-$($_.tag_name -replace '^v','').windows-x64.zip"
		if ([version]$version -gt [version]'5.1') {$url = "https://s3-us-west-2.amazonaws.com/grafana-releases/release/grafana-$($_.tag_name -replace '^v','').windows-amd64.zip"}
