...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/popcorn-official/popcorn-desktop/tags?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
			[version]$_.name
		}
		catch {
		...
		$url = "https://get.popcorntime.sh/build/Popcorn-Time-$($_.name)-Setup.exe"
