...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/papertrail/remote_syslog2/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
		...
		$url = $_.assets.browser_download_url -match 'remote_syslog_windows_i386.tar.gz'
		$url64 = $_.assets.browser_download_url -match 'remote_syslog_windows_amd64.tar.gz'
