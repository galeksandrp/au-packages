...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/gogs/gogs/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
        ...
        $url = $_.assets.browser_download_url -match 'windows_386.zip'
		$url64 = $_.assets.browser_download_url -match 'windows_amd64.zip'
