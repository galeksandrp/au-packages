...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/deemru/stunnel/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
			[version]($_.tag_name -replace 'stunnel-','' -replace '-msspi-','.')
		}
		catch {
		}
		...
		$version = [version]($_.tag_name -replace 'stunnel-','' -replace '-msspi-','.')
		$version = $version.major, $version.minor -join '.'
		if (!$versions[$version]) {
		
		$url = ($_.assets -match '_windows-386.zip').browser_download_url
		$url64 = ($_.assets -match '_windows-amd64.zip').browser_download_url
