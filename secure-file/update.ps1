...
function global:au_GetLatest {
	...
	(curl "https://api.github.com/repos/appveyor/secure-file/releases?client_id=$env:GITHUB_CLIENT_ID&client_secret=$env:GITHUB_CLIENT_SECRET" | ConvertFrom-Json) | ? { #filter out nuspec-noncompatible versions. Filter is applied here, and not in foreach because it is needed immediately in next sort
		try {
			[version]$_.tag_name
		}
		catch {
